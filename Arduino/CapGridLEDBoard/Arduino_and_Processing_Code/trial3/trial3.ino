...
int sensors[] = {5,0,7,2};  // The sensor at index 0 is the most sensitive

byte calibration[numSensor];
float normalizationVal[numSensor];
float sensorRange = 300;
float scale = 1;

float sensorRanges[] = {scale*350, scale*350, scale*350, scale*350};    // Hooked up to computer settings
int light_threshold = 175; // 175 works in the lab

//float sensorRanges[] = {350, 350, 225, 350};
//int light_threshold = 20; // 175 works in the lab
...
/*
  ...
  */
  ...
  for(int i = 0; i < numSensor; i++){
      ...
      setNormalizationVal(i);
      calibrate(i);
      setNormalizationVal(i);
  }
  float temp = normalizationVal[0];
  normalizationVal[0] = normalizationVal[3];
  normalizationVal[3] = normalizationVal[2];
  normalizationVal[2] = normalizationVal[1];
  normalizationVal[1] = temp;
  

}
void setNormalizationVal(int index){
  normalizationVal[index] = 0;
  int numSample = 10;
  for(int i = 0; i<numSample; i++){
    ...
    // jigger
      ...
      if(prevState == 1 && currState == 0){
        ...
        unsigned long code = readValue();
        Serial.println(code);
        normalizationVal[index] += 1000*(4.096*2.0*(code))/(1.0 * 0xffffffUL);
        happen = true;
      }
    prevState = currState; 
    }
  }
  normalizationVal[index]/=numSample;
  Serial.print("normalization: ");
  Serial.println(normalizationVal[index]);
}
boolean on = false;
void loop()
{
...
//              digitalWrite(7,LOW);
  ...
  String outString = String(int(1000*(4.096*2.0*(codes[0]))/(1.0 * 0xffffffUL)-4.096)) + "," + 
                      ...
                      String(int(1000*(4.096*2.0*(codes[2]))/(1.0 * 0xffffffUL)-4.096)) + "," +
                      String(int(1000*(4.096*2.0*(codes[3]))/(1.0 * 0xffffffUL)-4.096));
...
void updateLEDs(uint32_t c, uint8_t wait,unsigned long code[]) {
  for(int j = 0; j < numSensor; j++){
    float val = 1000*(4.096*2*(code[j]))/(1.0 * 0xffffffUL);
     ...
     //uint32_t newC = strip.Color(range, 255-range, 0);
...
void calibrate(int index) 
{
  ...
  while (value>VALUE_UPPER_BOUND && calibration[index] < 128) {
    calibration[index]++;
    writeConfig(REGISTER_CAP_DAC_A, _BV(7) | calibration[index]);
    value = readValue();
  }
