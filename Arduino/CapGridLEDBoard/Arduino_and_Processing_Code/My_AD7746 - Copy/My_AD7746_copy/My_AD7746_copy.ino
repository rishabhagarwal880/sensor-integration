...
int sensors[] = {0};  // The sensor at index 0 is the most sensitive

//  sensors[0] = 0;  //3,4,5,6 //0
  ...
  */
  ...
  while(!happen){
    ...
    if(prevState == 1 && currState == 0){
      ...
      Serial.println(code);
      normalizationVal = 1000*(4.096*2.0*(code))/(1.0 * 0xffffffUL);
  ...
  Serial.println(normalizationVal);
...
void loop()
{
  ...
  if(prevState == 1 && currState == 0){
    ...
    //modifyCalibration(code);
    //Serial.println("Calibration: " + calibration);

    
    float capVal = 1000*(4.096*2*(code))/(1.0 * 0xffffffUL);
    //Serial.println("Cap Value: "+capVal);
    String outString = String(int(1000*(4.096*2.0*(code))/(1.0 * 0xffffffUL)-4.096)) + "," + String(calibration);
    Serial.println(outString);
    
    colorWipe(strip.Color(0, 255, 0), 50,capVal); // Red
  }
  prevState = currState;
...
/*
  Increment or decrement calibration value
*/
...
/*
  Calibration performed during void setup()
...
/*
  while (value<VALUE_LOWER_BOUND && calibrationB < 128) {
    calibrationB++;
    writeConfig(REGISTER_CAP_DAC_B, _BV(7) | calibrationB);
    value = readRegister(REGISTER_CAP_DATA,3);
  }
  ...
  */
  Serial.println("done");
     // writeConfig(REGISTER_CAP_DAC_A, _BV(7) | _BV(6) |  _BV(5));
