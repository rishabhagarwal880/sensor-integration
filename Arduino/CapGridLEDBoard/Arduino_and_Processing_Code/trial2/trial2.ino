...
const int numSensor = 3;
int sensors[] = {0,1,7};  // The sensor at index 0 is the most sensitive

byte calibration[numSensor];
float normalizationVal[numSensor];
float sensorRange = 150;
...
void loop()
{
  ...
  String outString = String(int(1000*(4.096*2.0*(codes[0]))/(1.0 * 0xffffffUL)-4.096)) + "," + String(int(1000*(4.096*2.0*(codes[1]))/(1.0 * 0xffffffUL)-4.096));
...
void updateLEDs(uint32_t c, uint8_t wait,unsigned long code[]) {
  for(int j = 0; j < numSensor; j++){
    float val = 1000*(4.096*2*(code[j]))/(1.0 * 0xffffffUL);
     ...
     int range = int((normalizationVal[j]-val)/(sensorRange)*255);
     if(range > 255){  
       range = 255; 
     }
     if(range < 0){  
     ...
     /*for(uint16_t i=0; i<strip.numPixels()/4; i++) {
         if(range > 0){
           strip.setPixelColor(4*i+2*j, c);
           strip.setPixelColor(4*i + 1+2*j, c);
           range--;
           strip.show();
           delay(wait);  
         }else{
